...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  if (last) {
    ...
    if (bookData[last.chapter]) showChapter(last.chapter);
    else showChapter(1);
  } else {
...
async function loadBook(book) {
  try {
    ...
    generateChapters();
    console.log(`Книга "${book.name}" загружена успешно.`);
    return true;
  } catch (e) {
...
function showChapter(chapterNum) {
  if (!selectedBook || !bookData || !bookData[chapterNum]) return;

  currentChapter = chapterNum;
  const verses = bookData[chapterNum];
  let html = `<h2>${selectedBook.name}, ${chapterNum}</h2>`;
  verses.forEach(v => {
